...
import { MatchType, MatchLevel, Match } from "@/store/matchStore";
...
export default function EditMatchScreen() {
  ...
  const { getMatchDetails, updateMatch, isLoading, error } = useMatches();
  ...
  // Cargar datos del partido y canchas disponibles
  useEffect(() => {
    const loadData = async () => {
      if (id) {
        try {
          ...
          if (user?.uid !== matchData.createdBy) {
            showNotification(
          ...
          setMatchType(matchData.type);
          setMatchLevel(matchData.level || "all");
          setIsPrivate(matchData.isPrivate || false);
          setUniformA(matchData.uniformA || "");
          setUniformB(matchData.uniformB || "");
          setDescription(matchData.description || "");

          // Cargar canchas
          await fetchFields(true);

          // Si hay una cancha seleccionada, buscarla en la lista
          if (matchData.fieldId) {
            const field = await getFieldDetails(matchData.fieldId);
  ...
  const getFieldDetails = async (fieldId: string): Promise<Field | null> => {
    ...
    try {
      // Si no está en la lista cargada, intentar obtenerla directamente
  ...
  // Manejar actualización del partido
  const handleUpdateMatch = async () => {
    if (!user || !profile || !match) {
      showNotification("Debes iniciar sesión para editar un partido", "error");
      return;
    }
    ...
    try {
      ...
      // Crear objeto con los datos actualizados del partido
      const updatedData = {
      ...
      // Actualizar partido
      await updateMatch(match.id, updatedData);
      ...
      showNotification(
        `Error al actualizar partido: ${(error as Error).message}`,
        "error"
      );
    }
  };
      ...
      <SafeAreaView edges={["top"]} style={styles.safeArea}>
        ...
        >
          <View style={styles.header}>
            ...
            <ThemedText type="title">Editar Partido</ThemedText>
          </View>
